...
class EncoderUNetBlock(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask_downsampled.size(2) != x3.size(2):
            if mask_downsampled.size(2) < x3.size(2):
                mask_downsampled = F.pad(
                    mask_downsampled, (0, x3.size(2) - mask_downsampled.size(2))
                )
            else:
...
class Encoder(nn.Module):
    def __init__(self, filters, kernels, first_block_in_channels=None):
        ...
        blocks.append(
            EncoderUNetBlock(
                filters[0], kernels[0], in_channels=first_block_in_channels
            )
        )

        for i in range(1, len(filters)):
            blocks.append(
                EncoderUNetBlock(filters[i], kernels[i], in_channels=filters[i - 1])
            )
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            enc_filters, enc_kernels, first_block_in_channels=pre_lstm_units
        )
    ...
    def forward(self, x, mask):
        ...
        )  # Element-wise multiplication to mimic TF's 'mul' merge mode
        ...
        packed_decoder = nn.utils.rnn.pack_padded_sequence(
            decoder_output, lengths=seq_lengths, batch_first=True, enforce_sorted=False
        )
        packed_post_lstm, _ = self.post_lstm(packed_decoder)
        post_lstm, _ = nn.utils.rnn.pad_packed_sequence(
            packed_post_lstm, batch_first=True
        )
