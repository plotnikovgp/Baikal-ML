...
class AngleUncertaintyPlotter(BasePlotter):
    def _plot_coordinate_uncertainties(self, data: dict):
        ...
        for i, (ax, coord) in enumerate(zip(axs, coord_labels)):
            ...
            ax.set_ylabel(
                f"{coord} - {coord}*", fontsize=LABEL_FONT_SIZE, fontweight="bold"
            )
            cbar = fig.colorbar(l[3], ax=ax)
    ...
    def _plot_angle_uncertainties(self, data: dict):
        ...
        for i, (sigma, error, symbol, title) in enumerate(angle_data):
            ...
            valid_mask = ~(
                np.isnan(sigma) | np.isnan(error) | np.isinf(sigma) | np.isinf(error)
            )
            ...
            norm = (
                colors.LogNorm(vmin=1e-5)
                if symbol == "phi"
                else colors.LogNorm(vmin=1e-4)
            )
            l = ax.hist2d(
                sigma_valid,
                error_valid,
            ...
            ax.set_ylabel(
                f"{symbol} - {symbol}*", fontsize=LABEL_FONT_SIZE, fontweight="bold"
            )
    ...
    def _plot_cmp(self, x, y, ax, title):
        ...
        ax.set_xlabel(
            rf"$\log \sigma_{{\mathrm{{pred}},\,{title}}}$", fontweight="bold"
        )
        ax.set_ylabel(
            rf"$\log \sigma_{{\mathrm{{true}},\,{title}}}$", fontweight="bold"
        )
    ...
    def _plot_coordinate_uncertainty_metrics(self, data: dict):
        ...
        fig.suptitle(
            "Coordinate Uncertainty Comparison", fontsize=16, fontweight="bold"
        )
