...
def load_model_predictions(checkpoint_path, config, cache_dir, no_cache, batch_size, model_name):
    ...
    print(f"{'=' * 60}")
    model, loaded_config = load_checkpoint_and_config(
        checkpoint_path, config if isinstance(config, str) else None
    )
    ...
    for i, (loader, name) in enumerate(zip(val_loaders, dataset_names)):
        ...
        if is_unlabeled:
            ...
            metrics = binary_clf_metrics(preds, labels, min_recall=0.9)
